export const blogPostTranslations = {
    en: {
      // Navigation
      'nav.backToNews': 'Back to News',
      
      // Article Meta
      'meta.category': 'Events',
      'meta.date': 'March 15, 2024',
      'meta.experts': 'International and Domestic Experts Gathered',
      
      // Article Title
      'article.title': 'Expert Meeting on Neuroblastoma Treatment',
      
      // Main Content
      'content.paragraph1': 'We are extremely proud to have supported and sponsored an expert meeting where the main topic was the treatment of high-risk neuroblastoma. Domestic and international experts were presented with the latest research in this field and previous experiences were shared to improve the future application of therapy for this rare disease.',
      'content.paragraph2': 'We gathered all parties dealing with neuroblastoma in one place to socialise and share knowledge with the aim to further improve the treatment of patients in Serbia.',
      
      // Key Focus Section
      'keyFocus.title': 'Key Focus',
      'keyFocus.content': 'Neuroblastoma is the leading cancer‐related cause of death in children, and the targeted cancer immunotherapy we distribute reflects our commitment to work towards equity in social opportunity, healthcare, and access to diagnosis and therapies for people living with a rare disease.',
      
      // Gallery
      'gallery.title': 'Event Gallery',
      'gallery.image1Alt': 'Expert Meeting Image 1',
      'gallery.image2Alt': 'Expert Meeting Image 2',
      'gallery.image3Alt': 'Expert Meeting Image 3',
      'gallery.image4Alt': 'Expert Meeting Image 4',
      'gallery.image5Alt': 'Expert Meeting Image 5',
      'gallery.imageLectureAlt': 'Expert Meeting Lecture',
      
      // Share
      'share.button': 'Share this Article',
      
      // Related Articles
      'related.title': 'Related Articles',
      'related.article1.title': 'New Partnership Expands Access to Orphan Drugs',
      'related.article1.date': 'March 10, 2024',
      'related.article2.title': 'Advancing Healthcare Through Innovation',
      'related.article2.date': 'March 5, 2024',
      'related.readMore': 'Read More →'
    },
    sr: {
      // Navigation
      'nav.backToNews': 'Nazad na Vesti',
      
      // Article Meta
      'meta.category': 'Događaji',
      'meta.date': '15. mart 2024.',
      'meta.experts': 'Okupljeni Međunarodni i Domaći Stručnjaci',
      
      // Article Title
      'article.title': 'Stručni Sastanak o Lečenju Neuroblastoma',
      
      // Main Content
      'content.paragraph1': 'Izuzetno smo ponosni što smo podržali i sponzorisali stručni sastanak na kojem je glavna tema bila lečenje visokorizičnog neuroblastoma. Domaćim i međunarodnim stručnjacima predstavljena su najnovija istraživanja u ovoj oblasti i podeljena su prethodna iskustva kako bi se poboljšala buduća primena terapije za ovu retku bolest.',
      'content.paragraph2': 'Okupili smo sve strane koje se bave neuroblastomom na jednom mestu radi druženja i razmene znanja sa ciljem daljeg unapređenja lečenja pacijenata u Srbiji.',
      
      // Key Focus Section
      'keyFocus.title': 'Ključni Fokus',
      'keyFocus.content': 'Neuroblastom je vodeći uzrok smrti povezane sa rakom kod dece, a ciljana imunoterapija raka koju distribuiramo odražava našu posvećenost radu ka jednakosti u društvenim mogućnostima, zdravstvenoj zaštiti i pristupu dijagnostici i terapijama za ljude koji žive sa retkom bolešću.',
      
      // Gallery
      'gallery.title': 'Galerija Događaja',
      'gallery.image1Alt': 'Slika 1 sa Stručnog Sastanka',
      'gallery.image2Alt': 'Slika 2 sa Stručnog Sastanka',
      'gallery.image3Alt': 'Slika 3 sa Stručnog Sastanka',
      'gallery.image4Alt': 'Slika 4 sa Stručnog Sastanka',
      'gallery.image5Alt': 'Slika 5 sa Stručnog Sastanka',
      'gallery.imageLectureAlt': 'Predavanje sa Stručnog Sastanka',
      
      // Share
      'share.button': 'Podeli Članak',
      
      // Related Articles
      'related.title': 'Povezani Članci',
      'related.article1.title': 'Novo Partnerstvo Proširuje Pristup Lekovima za Retke Bolesti',
      'related.article1.date': '10. mart 2024.',
      'related.article2.title': 'Unapređenje Zdravstvene Zaštite Kroz Inovacije',
      'related.article2.date': '5. mart 2024.',
      'related.readMore': 'Pročitaj Više →'
    }
  };